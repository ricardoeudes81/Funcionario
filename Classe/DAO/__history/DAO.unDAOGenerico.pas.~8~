...
type
  ...
  ['{7913DFD3-1474-4817-8CBD-E2D664A399E0}']
    ...
    function ObterListaComFiltro(CampoFiltro: string; ValorFiltro: Variant): TObjectList<T>;
  end;
...
function TDAOGenerico<T>.ObterListaComFiltro(CampoFiltro: string;
  ValorFiltro: Variant): TObjectList<T>;
var
  Conn: iSimpleQuery;
  DAOGenerico: iSimpleDAO<T>;
begin
  Result := TObjectList<T>.Create;
  if (CampoFiltro.IsEmpty) or (VarToStr(ValorFiltro).IsEmpty) then
    Exit;
  ...
  DAOGenerico.SQL
      .Fields('*')
      .Where(CampoFiltro + ' = ' + VarToStr(ValorFiltro))
      .OrderBy('ID')
      .&End
    .Find
